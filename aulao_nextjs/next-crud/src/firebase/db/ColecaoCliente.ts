...
export default class ColecaoCliente implements ClienteRepositorio{
    ...
    async salvar(cliente: Cliente): Promise<Cliente> {
        ...
        //cenário 1: se o cliente.id existir fignifica que vou alterar
        //senão siginifica que irei acidionar um novo cliente

        if(cliente?.id){
            //altera um cliente
            const taskDocRef = doc(db,'pessoas',cliente.id)
            try{
